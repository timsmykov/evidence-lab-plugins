...
1. **Forward citations** (papers citing key papers):
   - Use the Semantic Scholar or OpenAlex citation routes documented in `paper-lookup`
   - Identifies newer research building on seminal work

2. **Backward citations** (references from key papers):
   - Retrieve authorized full text through `paper-lookup` or the supplied local document set
