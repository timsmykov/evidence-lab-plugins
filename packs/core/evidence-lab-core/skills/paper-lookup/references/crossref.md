...
None required. Add `mailto=CONTACT_EMAIL` to get into the **polite pool** (2x faster rate limits).
...
```
GET /works?query={text}&rows={n}&mailto=CONTACT_EMAIL
```
...
**Example:**
```
https://api.crossref.org/works?query=CRISPR+gene+therapy&filter=from-pub-date:2024-01-01,type:journal-article,has-abstract:true&rows=5&sort=published&order=desc&mailto=CONTACT_EMAIL
```

### 2. Get work by DOI

```
GET /works/{doi}?mailto=CONTACT_EMAIL
```
...
**Example:**
```
https://api.crossref.org/works/10.1038%2Fnature12373?mailto=CONTACT_EMAIL
```
