...
No API key. You must include your **email address** as a query parameter: `?email=CONTACT_EMAIL`

**Important:** Supply an authorized real contact through `CONTACT_EMAIL`. Unpaywall rejects placeholder contact values with HTTP 422.
...
```
GET /v2/{doi}?email=CONTACT_EMAIL
```

**Example:**
```
https://api.unpaywall.org/v2/10.1038/nature12373?email=CONTACT_EMAIL
```

### 2. Search (unreliable)

```
GET /v2/search?query={text}&email=CONTACT_EMAIL
```
